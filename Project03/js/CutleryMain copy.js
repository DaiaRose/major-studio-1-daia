...
function cleanData(data) {
  ...
  //console.log(`Total cleaned items: ${cleanedData.length}`);
    ...
    if (cutlery.length > 0) {
      ...
      cutlery.forEach(item => {
        item.materials.forEach(material => {
          ...
          groupedData[material][item.type]++; 
        });
      });
      ...
      //console.log("Scatter Plot Data:", filteredScatterPlotData); 
...
yAxis.selectAll("text")
  .attr("dy", "0.5em") 
  .attr("dx", "-1.8em") // Adjust horizontal position to out
  .attr("font-size", "12px") 
...
const labels = xAxis.selectAll("text")
  ...
  const circles = g.selectAll("circle")
    ...
    .attr("opacity", d => {
      ...
      const logCount = Math.log(d.count + minCount); 
      const maxLog = Math.log(1080 + minCount); // Log of max count for scaling
      return Math.min(1, 0.1 + (logCount / maxLog) * 0.9); 
      })
    .on("mouseover", function(event, d) {
        d3.select("#tooltip")
            .style("visibility", "visible")
...
svg.append("text")
.attr("x", 512) 
.attr("y", 580) // Adjust vertical position
...
function resizeSVG() {
    ...
    svg.setAttribute('height', width / aspectRatio);
}

window.addEventListener('resize', resizeSVG);
resizeSVG(); // Call on load