...
function fetchUrl(searchAllURL){
  request(searchAllURL, function (error, response, body) {
    ...
    let objects = obj.response.rows.filter(data => {
      ...
      }//we're going shopping for id, title, primary image, filename
      // we should modify this
...
// to actually run in terminal: daiabromberg@daialaptop 05_node_image_download % node node_download_images.js